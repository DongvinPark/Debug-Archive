...
- DB 내 데이터 정합성 깨진 테이블 다수 존재
    - UNIQUE 제약이 필수인 테이블(chats_room)에는 특정 로우들 삭제 후 제약 추가
    - CASCADE 옵션 전면 비활성화 & row의 물리적인 삭제 일절 금지
    - 정합성이 깨진 테이블들 전수 조사 후 후임 개발자에게 인수인계
- API 서버(Python/Django)가 채팅을 위한 웹소켓 연결도 관리
    - 2.0 에서는 API 서버와 CHAT 서버를 별개의 인스턴스로 분리하여 역할과 트래픽을 분리시킴
    - CHAT 서버를 소켓 관련 커널 튜닝을 마친 EC2 인스턴스에서 돌림으로써, 대규모 웹소켓 동시 접속 상황에 대비함
    - API 서버 컨테이너가 배포/교체 돼도 웹소켓 연결이 끊어지지 않음
    - API 기능과 웹소켓 기능이 서로의 성능을 깎아먹는 것을 예방
- 네트워킹 작업을 동기식으로 처리
    - 2.0 에서는 FCM 알림 전송, Heaby DB Write의 처리가 API 응답을 Block 하지 않도록 @Asunc, CompletableFuture를 적극 사용
- '채팅이 늦게 뜬다'는 고객 컴플레인 존재
    - 1.0 에서는 채팅 메시지들을 항상 DB에서만 읽어옴
    - 2.0 에서는 레디스에 최신 메시지들을 채팅룸마다 최대 200 개까지 캐시해 둠으로써 DB의 부하를 감소
    - 메시지 수정/삭제가 발생할 시 레디스 내의 메시지 캐시를 삭제함으로써 캐시 & DB 간의 일관성 유지 로직을 단순화